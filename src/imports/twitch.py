...
class Twitch:
    ...
    async def check(self, streamerChannel: discord.TextChannel):
        for streamer in self.twitch.find():
            ...
            r = requests.get(f"https://api.twitch.tv/helix/streams?user_login={username}",
            ...
            if streamData["data"]:
                ...
                user    = self.bot.get_user(streamer["discord_id"])
                embed = discord.Embed(
                    title=streamData["title"], url=f"https://twitch.tv/{username}", color=0x8000ff)
                ...
                if not streamer["message_id"]:
                    self.u.log(
                        f"\t\t{username} is now live, announcing stream...")
